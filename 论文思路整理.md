...
3. **热门 Adapter 多副本**：检测热点并复制到多个 Worker，线性提升热点吞吐
...
- ⚠️ **未考虑 LoRA Rank**：当前路由不感知 Adapter 的 rank 大小
...
本文基于 S-LoRA 系统，提出面向高并发多租户场景的多 Worker 数据并行推理架构。主要贡献包括：(1) 多 Worker 数据并行架构，每个 GPU 独立运行完整模型实例，实现线性可扩展性；(2) Adapter 感知路由基于缓存亲和性和负载感知优化请求分发；(3) 热门 Adapter 多副本机制实现热点吞吐量线性扩展；(4) 多维评分驱动的双模式淘汰机制实现精细化显存管理。
...
$$Score_i(r) = w_1 \cdot \mathbf{I}(adapter \in Cache_i) - w_2 \cdot QueueLen_i$$

| 参数 | 含义 | 默认值 |
|------|------|--------|
| $w_1$ | 缓存亲和性权重 | 10.0 |
| $w_2$ | 负载惩罚权重 | 1.0 |
| $\mathbf{I}(\cdot)$ | 指示函数 | 0 或 1 |

**当前限制**：未考虑 LoRA Rank，可能导致大小 Rank 混合批处理效率低下

**扩展方向（Rank 感知）**：
$$Score_i(r) = w_1 \cdot \mathbf{I}(adapter \in Cache_i) - w_2 \cdot QueueLen_i - w_3 \cdot RankMismatch_i$$

其中 $RankMismatch_i$ 表示新请求的 Rank 与 Worker $i$ 当前批次 Rank 的差异
...
2. **Rank 感知路由** ⭐⭐⭐⭐⭐ - 避免大小 Rank 混合批处理，降低尾延迟（参考 Toppings）
3. **混合并行策略** ⭐⭐⭐⭐⭐ - TP + DP 结合
