...
@pytest.mark.asyncio
async def test_update_routing_config_accepts_token_count_active(tmp_path, monkeypatch):
    config_file = tmp_path / "slora_routing_config_update.json"
    real_open = open
    ...
    response = await update_routing_config(StubRequest({
        ...
        "reset_stats": True,
        "update_id": "test-token-count-active",
    }))

    assert response.status_code == 200
    assert json.loads(response.body)["config"]["load_metric"] == "token_count_active"
    assert json.loads(config_file.read_text())["load_metric"] == "token_count_active"
